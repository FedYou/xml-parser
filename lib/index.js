...
function validateAtts(atts) {
  if ((atts && typeof atts !== 'object') || Array.isArray(atts)) {
    err(`@att must be an object, got ${typeof atts}: ${atts}`)
  }
  for (const attKey in atts) {
    ...
    if (typeof att === 'object') {
      err(
        `@att[${attKey}] must be a string, number or boolean, got ${typeof att}: ${JSON.stringify(att)}`
      )
    }
    if (typeof att === 'string' && att.includes("'")) {
      err(`@att[${attKey}] The attribute must not contain single quotes, got ${typeof att}: ${att}`)
    }
  }
}
